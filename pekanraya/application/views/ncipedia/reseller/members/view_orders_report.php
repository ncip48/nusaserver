                ...
                <tbody>
                  <?php 
                    ...
                    if ($row['proses']=='0'){ 
                      ...
                      $proses = '<i class="text-succress">Dikirim</i>'; 
                    }else{ 
                    ...
                    if($qwer[total]=='0'){
                      ...
                      $total = $this->db->query("SELECT a.diskvoucher-a.diskon as total FROM `rb_penjualan_detail` a where a.id_penjualan='$row[id_penjualan]'")->row_array();
                    }

                    #$total = $this->db->query("SELECT sum((a.harga_jual*a.jumlah)-a.diskon) as total FROM `rb_penjualan_detail` a where a.id_penjualan='$row[id_penjualan]'")->row_array();
                    #$total = $this->db->query("SELECT sum(a.diskvoucher-a.diskon) as total FROM `rb_penjualan_detail` a where a.id_penjualan='$row[id_penjualan]'")->row_array();
                    echo "<tr><td>$no</td>
