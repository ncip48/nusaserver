	...
	$kategori = $this->model_app->view('rb_kategori_produk');
		...
		$sub_kategori = $this->db->query("SELECT * FROM rb_kategori_produk_sub where id_kategori_produk='$rows[id_kategori_produk]'");
		    if ($sub_kategori->num_rows()>=1){
			    echo "<li><a class='label-name' href='".base_url()."produk/kategori/$rows[kategori_seo]'> $rows[nama_kategori]</a>
				      <ul>";
				    foreach ($sub_kategori->result_array() as $row) { 
					    echo "<li><a class='label-name' href='".base_url()."produk/subkategori/$row[kategori_seo_sub]'>$row[nama_kategori_sub]</a></li>";
					}
					    echo "</ul></li>";
			}else{
				echo "<li><a class='label-name' href='".base_url()."produk/kategori/$rows[kategori_seo]'> <a href='".base_url()."produk/kategori/$rows[kategori_seo]'> $rows[nama_kategori]</a></li>";
			}
		}
echo "</ul>";
?>
...
<ul>
    <li>
        <a class='label-name' href='https://bospediaku.blogspot.com/search/label/Hijab'>Hijab
        <span class='label-count'>18</span>
        </a>
    </li>
</ul>
...
<!-- search-populart -->
<div class='search-content section' id='search-content' name='Pencarian'>
	<div class='widget Label' data-version='2' id='Label4'>
		<div class='widget-title'>
			<h3 class='title'>Pencarian Populer</h3>
		</div>
		<div class='widget-content cloud-label'>
			<ul><li><a class='label-name' href='https://bospediaku.blogspot.com/search/label/Hijab'>Hijab</a></li></ul>
		</div>
	</div>
</div>
</div>
