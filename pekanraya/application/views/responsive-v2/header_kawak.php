<div class='subheader'>
    <div class='subheader-inner'>
        <div class='subheader-left'>
            <a href='/'>Download Aplikasi PekanRaya</a>
        </div>
        <div class='subheader-right'>
            <a href='#'>Mulai Berlanja</a>
            <a href='#'>Promo</a>
            <a href='#'>Testimoni</a>
            <a href='#'>Pusat Bantuan</a>
        </div>
    </div>
</div>

<header id='header-wrapper'>
    <div class='header section' id='header'>
        <div class='widget Header' data-version='2' id='Header1'>
            <div class='header-widget'>
                <?php 
                $iden = $this->model_utama->view('identitas')->row_array();
                $logo = $this->model_utama->view_ordering_limit('logo','id_logo','DESC',0,1);
                foreach ($logo->result_array() as $row) {
                echo "<a class='main-logo-img' href='".base_url()."'><img class='lazyload' data-height='90' data-width='272' src='".base_url()."asset/logo/$row[gambar]'/></a>";
                }
                ?>
            </div>
        </div>
    </div>
<!-- MENU LINK -->
    <a class='icon_menu' href='javascript:void(0);' onclick='openNav();menuoverlayOn();addClassBody();' title='Menu'></a>
    <div class='cssmenu' id='mynav' itemscope='itemscope'>
        <div class='dropdown'>
            <button class='dropbtn'>Kategori
                <i class='fa fa-angle-down'></i>
            </button>
            <div class='dropdown-content section' id='dropdown-content'>
                <div class='widget Label' data-version='2' id='Label1'>
                    <div class='widget-title'>
                        <h3 class='title'>Label</h3>
                    </div>
                    <div class='widget-content list-label'>
                        <?php 
                        $kategori = $this->model_app->view('rb_kategori_produk');
                        $jumlah = $this->db->query("SELECT COUNT(*) FROM rb_produk WHERE id_kategori_produk=16");
	                    foreach ($kategori->result_array() as $rows) {
                        $h = $this->db->query("SELECT COUNT(*) as jumlah FROM rb_produk WHERE id_kategori_produk=$rows[id_kategori_produk]")->row_array();
	                    echo "<ul><li><a class='label-name' href='".base_url()."produk/kategori/$rows[kategori_seo]'>$rows[nama_kategori]<span class='label-count'>$h[jumlah]</span></a></li>";
	                    }	
                        echo "</ul><a class='allProduct' href='".base_url()."produk'><b>Semua</b> Produk <i class='fa fa-angle-right'></i></a>" ?>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <a class='closebtn' href='javascript:void(0)' onclick='closeNav()'></a>
    <div id='menuoverlay' onclick='closeNav();menuoverlayOff();removeClassBody()'></div>

<!-- MENU SEARCH -->
<div class='search-icon'>
	<?php echo form_open('produk/index') ?>
		<input autocomplete='off' name='kata' class='input-search' placeholder='Cari produk atau toko' type='text' value=''/>
		<input name='max-results' type='hidden' value='0'/>
		<i class='rianseo-tutup'></i>
		<button class='button'><i></i></button>
	</form>
<!-- search-populart -->
</div>

<!-- MENU SEARCH
    <div class='search-icon'>
        <?php echo form_open('produk/index') ?>
            <input autocomplete='off' class='input-search' name='kata' placeholder='Cari produk atau toko' type='text' required/>
            <input name='max-results' type='hidden' value='0'/>
            <i class='rianseo-tutup'></i>
            <button class='button'><i></i></button>
        </form>
        </div>
    </div> -->
    
<!-- MENU LINK KANAN -->
    <div class='link-right'>
        <div class='toko-profil section' id='toko-profil'>
            <div class='widget HTML' data-version='2' id='HTML7'>
                <div class='widget-title'>
                    <h3 class='title'>Profil Admin</h3>
                </div>
                <div class='widget-content'>
                    <a href='#'><img class='lazyload' itemprop='image' src='data:image/png;base64,'/></a>
                    <ul class='dropdown'>
                        <?php if($this->session->id_konsumen==''){
                        echo "<li><a href='".base_url()."auth/login'>Login</a></li>
                        <li><a href='".base_url()."auth/register'>Register</a></li>";
                        }else{
                        echo "<li><a href='".base_url()."members/profile'>Profile</a></li>";
                        echo "<li><a href='".base_url()."members/edit_profile'>Edit Profile</a></li>";
                        echo "<li><a href='".base_url()."members/voucher'>Voucher</a></li>";
                        echo "<li><a href='".base_url()."members/logout'>Logout</a></li>";
                        }
                        ?>
                    </ul>
                </div>
            </div>
        </div>
        <?php echo "<li><a href='".base_url()."members/keranjang'><i class='fa fa-shopping-cart fa-lg'></i></a></li>"; ?>
    </div>
<div class='clear'></div>
</header>