...
<?php 
  ...
  if ($stok=='0'){
    $blur = 'blur'; 
    $status = 'SOLD OUT';
    $btn = "<a class='btn btn-biru text-white mt-4 btn-disabled'>Beli</a>"; 
  }else{ 
    $blur = 'normal'; 
    $status = 'STOK ADA';
    $btn = "<a target='_BLANK' class='btn btn-biru text-white mt-4' href='https://api.whatsapp.com/send?phone=$iden[no_wa]&text=Halo $iden[nama_website], Saya mau membeli $row[nama_produk], Apakah Masih Tersedia?'>Beli</a>"; 
  }
  echo "
  <!-- Portfolio Item Row -->
  <div class='row'>
