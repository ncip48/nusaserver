...
class Breadcrumb{
 ...
 public function show($home_site ="Home >", $id = "crumbs"  ) 
 {
  ...
  if ($breadcrumbs && count($breadcrumbs) > 0) {
   $out .= '<li><a class="pathway small" href="' . base_url() .'"/>'. $site . '</a></li>';
   $i=1;
   foreach ($breadcrumbs as $crumb): 
 
    if ($i != count($breadcrumbs)) {
     $out .= $sep . '<li><a class="pathway small" href="' .site_url($crumb['uri']). '">'. $crumb['title'] .' > </a></li>';
    } else {
     $out .= $sep . '<li><a class="pathway small">'. $crumb['title'] .'</a></li>';
    }
    $i++;
   endforeach;
  } else {
